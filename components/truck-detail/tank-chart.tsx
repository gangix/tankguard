...
export default function TankChart({ readings }: { readings: Array<{ recorded_at: string; liters: number }> }) {
  const formatter = new Intl.DateTimeFormat("en-GB", { timeZone: "Europe/Istanbul", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" });
  const data = readings.map((reading) => ({ ...reading, label: formatter.format(new Date(reading.recorded_at)) }));
